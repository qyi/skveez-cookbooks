...
include_recipe "postgresql::client" if configure_options =~ /pgsql/
...
pkgs = value_for_platform(
    ...
    [ "debian", "ubuntu" ] =>
        {"default" => %w{ libbz2-dev libc-client2007e-dev libcurl4-gnutls-dev libfreetype6-dev libgmp3-dev libjpeg62-dev libkrb5-dev libmcrypt-dev libpng12-dev libssl-dev libt1-dev libicu48 libicu-dev}},
...
bash "build php" do
  ...
  (cd php-#{version} && make)
  (cd php-#{version} && checkinstall --default --pakdir #{node['checkinstall']['pakdir']} -D make install)
  EOF
  not_if "which php"
end
...
template "#{node['php']['fpm_conf_dir']}/pool.d/www.conf" do
  source "www.conf.erb"
  owner "root"
  group "root"
  mode "0644"
...
link "#{node['php']['fpm_conf_dir']}/conf.d" do
  to node['php']['ext_conf_dir']
end
