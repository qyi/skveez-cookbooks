...
unless(node['nginx']['source']['prefix'])
  node.set['nginx']['source']['prefix'] = "/usr/share/nginx"
end
...
#fix for recompile remove parameters
%w{fastcgi_params uwsgi_params fastcgi.conf scgi_params}.each do |conf|
  file "#{node['nginx']['dir']}/#{conf}" do
    action :delete
    only_if { File.exists? "#{node['nginx']['dir']}/#{conf}" }
    not_if do
